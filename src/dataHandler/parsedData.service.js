...
angular.module('evtviewer.dataHandler')
	...
	.service('parsedData', ['$log', 'config', 'Utils', function ($log, config, Utils) {
	   ...
	   */
		...
		/**
		 * @ngdoc method
		 * @name evtviewer.dataHandler.parsedData#addTeiHeader
		 * @methodOf evtviewer.dataHandler.parsedData
		 *
		 * @description
		 *
		 * @param {object} teiHeader the JSON object with all the info about the parsed teiHeader
		 * @param {string} docId the id of the document the teiHeader belongs to
		 *
		 * @author CM
		 */
		parsedData.addTeiHeader = function (teiHeader, docId) {
			var teiHeaderId = teiHeader.value;
			teiHeaderCollection[teiHeaderCollection.length] = teiHeaderId;
			teiHeaderCollection[teiHeaderId] = teiHeader;
			teiHeaderCollection.length++;

			documentsCollection[docId].teiHeader = teiHeaderId;

		};

		parsedData.getTeiHeaders = function () {
			return teiHeaderCollection;
		}

		/**
		 * @ngdoc method
		 * @name evtviewer.dataHandler.parsedData#getTeiHeader
		 * @methodOf evtviewer.dataHandler.parsedData
		 *
		 * @description
		 * Get the object representing a particular page.
		 * @param {stirng} pageId Identifier of the teiHeader to retrieve
		 * @returns {Object} Object representing the page with <code>id = teiHeader</code>.
		 * It is structured as follow:
			  <pre>
				var teiHeader = {
					value,
		 ...
		 */
		parsedData.getTeiHeader = function (teiHeaderId) {
			return teiHeaderCollection[teiHeaderId];
		};
